...
public class PageCreator {
	...
	public PageCreator(Page page , int articleTotalCount) {
		...
		caclDataOfPage(); //전달 완료후 알고리즘 수행!
	}
	
	private void caclDataOfPage() {
		...
		this.next = this.articleTotalCount <= (this.end * page.getAmount()) ? false : true;
		
		if(!this.next) {
			this.end = (int) Math.ceil(this.articleTotalCount/(double) page.getAmount());
		}
