...
//		mv.addObject("name","value"); data 를 담고 싶을때
	...
	public String upload(SnsBoardRequestDTO dto) {
		System.out.println("SnsBoardController 는 들어오는가 :" + dto);
		service.insert(dto);
...
//		System.out.println("/snsbloard/getList : get !!");
		log.info("/snsbloard/getList : get !!");
...
//		System.out.println("fileName : " + fileName);
		...
		//응답 내용,응답이 성공했는지에 대한 여부,웅답에 관련된 여러 설정들을 지원합니다.
		...
		try {
			...
			//ResponseEntity 객체에 전달하고자 하는 파일을 byte[]로 변환헤서 전달.(파일의 손상을 막기 위해)
			//header 내용도 같이 포함,응답상태 코드를 원하는 형태로 전달이 가능.
	...
	public ResponseEntity<?> delete(@PathVariable int bno , HttpSession session) {
		...
		//메서드 delete() -> return type 이 boolean , 삭제 성공시 true,실패 시 false.
	...
	//회원이 글 목록으로 진입시 좋아요 게시물 list 체크
