...
public interface IReplyMapper {
	...
	/*
	 - MyBatis 로 DB 연동을 진행할 때, 파라미터 값이 2개 이상일 때 그냥 보내시면
	 에러가 발생하기 때문에 조치가 필요합니다.
	 
	 1.@Param 을 이용해서 이름을 붙여주는 방법.(mapper.xml에 해당 값을 지목할 수 있는 이름 붙이기)
	 2. Map 으로 포장해서 보내는 방법
	 3.클래스를 디자인해서 객체 하나만 매개값으로 보내는 방법
	 
	 중 하나를 상황에 맞게 적절하게 선택하면 됩니다.
	 */
	//List<Reply> getList(@Param("boardNo") int bno ,@Param("paging") Page page);//목록 요청
	
    List<Reply> getList(Map<String,Object> data);//목록 요청
	
	int getTotal(int bno); //댓글 개수(페이징,pageCreator 는 사용하지 않습니다.)
