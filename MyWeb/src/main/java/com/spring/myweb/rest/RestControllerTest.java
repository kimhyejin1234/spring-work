...
public class RestControllerTest {
	...
	/*
	 ...
	 서버에서 사용하눈 자바 언어에 맞게 변환하여 받을 수 있는 아노테이션.
	 @ResponseBody
	 -메서드가 리턴하는 데이터를 viewResolver 에게 전달히지 않고,
	 클라이언트에게 해당 데이터를 바로 응답하게 합니다.
	 비동기 통신에서 주로 많이 사용합니다.
	 @RestController
	 -아노테이션을 통해 빈으로 등록한 컨트롤러는
	 모든 메서드가 리턴 값을 클라이언투로 직접 리턴합니다.(viewResolver 를 사용하지 않습니다).
	 REST 방식의 통신 전용 컨트롤러 로 빈을 등록하는 것입니다.
	 */
	@PostMapping("/object")
//	@ResponseBody
