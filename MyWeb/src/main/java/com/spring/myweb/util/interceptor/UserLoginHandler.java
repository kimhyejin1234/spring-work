...
public class UserLoginHandler implements HandlerInterceptor {
	...
	//postHandle은 컨트롤러를 나갈때 공통 처리해야 할 내용을 작성
	// /userLogin 이라는 요청이 컨트롤러에서 마무리 된 후, viewResolver 로 전달이 되기 전
	// 로그인 성공 or 실패 여부에 따라 처리할 로직을 작성할 계획입니다.
	@Override
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
			ModelAndView modelAndView) throws Exception {
		System.out.println("로그인 인터셉터가 동작 합니다.");
		...
		if(request.getMethod().equals("POST")) {//요청방식이 POST 이면->로그인 요청이었다면,
			...
			if(result != null) {
				...
				response.sendRedirect(request.getContextPath() + "/"); //Context root 가 온다.
																	// 메인페이지로 이동
			} else {
				//로그인 실패
