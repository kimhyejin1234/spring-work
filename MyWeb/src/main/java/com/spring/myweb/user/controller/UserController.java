...
public class UserController {
    ...
    */
	...
	@ResponseBody // return 값을 경로가 아닌 view 화면에 직접 던진다.
	...
	public String join(UserJoinRequestDTO dto , RedirectAttributes ra) {
		...
		/*
		 ...
		 model 객체가 forward 상황에서 사용하는 request  의 대체체이기 때문에
		 redirect 를 통해 응듭이 나갔다가 재 요청이 들어오는 상황에서는 데이터가 소멸합니다.
		 (parameter에 노출되어 전달됨)
		 
		 redirect 상황에서 일회성으로 데이터를 정송할 때 사용하는 메서드 RedirectAttributes(name,value)
		 데이터가 url 에 노출되지 않고, 한 번 이용한 후에는 알아서 소멸합니다.
		 */
