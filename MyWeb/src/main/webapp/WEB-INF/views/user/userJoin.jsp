                ...
                <form action="${pageContext.request.contextPath}/user/join" method="post" name="joinForm">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group-addon">
                                <button type="button" onclick="searchAddress()" class="btn btn-primary">주소찾기</button>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
...
<script>
    ...
    document.getElementById("idCheckBtn").onclick = function () {
        ...
        */
        ...
        소비할 수 있게 해 줍니다.
        (Promise:비통기 통신의 결과 및 통신의 순서를 보장하는 객체)
        -fwrch 함수가 리턴하는 Promise 객체는 단순히 응답 JSON 데이터가 아닌
        전체적이고, 포괄적인 응답 정보를 가지고 있스빈다.
        ...
        //그 이후에 나중에 배치된 then()이 실행되게끔 메서드 체인형 방식으로 작성.
        ...
        fetch('${pageContext.request.contextPath}/user/id/' + userId)
            ...
            .then(data => { //텍스트 데이터만 담긴 Promise  객체로부터 data 를 전달받음
                if (data === 'ok') {
                    ...
                    //더이상 버튼을 누룰수 없도록 버튼 비활성화.
                    document.getElementById("idCheckBtn").setAttribute('disableed', true);
                    //메새지 넘기기
                    ...
                    document.getElementById("userId").focus(); //입력칸 포커싱.
                }
            });
    ...
    document.getElementById('mail-check-btn').onclick = function(){
        ...
        .then(data => {
            ...
            code = data ; //서버가 전달된 인증번호를 전역변수에 저장.
            alert('인증번호가 전송되었습니다.확인 후 입력란에 정확히 입력히세요');
        })
        .catch(error =>{
            console.log(error);
    ...
    document.querySelector('.mail-check-input').onblur = function(e){
        ...
        if(inputCode === code){
            ...
            //select 태그에서 초기값을 사용자가 선택합 값으로 무조건 설정하는 방밥
            //(sekect 에서 readonly대용으로 사용)
            //항상 2개 같이 쓰셔야 합니다.
            const $email2 = document.getElementById('userEmail2');
            
            //사용자가 select 의 옵션을 처음 선택했을 때의 값을 기억했다가
            //option 변경시도를 할 때마다 초기깂으로 강제로 변경해서
            //option 이 마치 변하지 않는 것처럼 처리.
    ...
    //다음 주소 api 사용해 보기(script src 추가 해야 합니다)
    ...
    id.onkeyup = function () {
        /*
            자바스크립트의 정규표현식 입니다  ..  /^[A-Za-z0-9+]{4,12}$/
