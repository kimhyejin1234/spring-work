                    ...
                    <div class="col-xs-12 col-md-9 write-wrap">
	                    ...
	                    <div class="reply-content">
	                        ...
	                        <div class="reply-group">
	                              ...
	                              <input type="text" id="replyId" class="form-control" placeholder="이름">
	                              <input type="password" id="replyPw" class="form-control" placeholder="비밀번호">
	                              </div>
                        ...
                        <div id="replyList">
                            <!--자바스크립트를 활용하여 반북문을 이용해서 댓글의 개수만틈 반복 표현
...
<script>
    ...
    window.onload = function(){
        document.getElementById('replyRegist').onclick = () => {
            ...
            fetch('${pageContext.request.contextPath}/reply',reqObj)
                ...
                .then(data => {
                    ...
                    //등록 완료후 댓글 목록 함수를 호출해서 비동기식으로 목록 표현
                    getList(1,true);
                });
        } // 댓글 등록 이벤트 끝.
        ...
        let strAdd = '';//화면에 그려 넣을 태그를 문자열의 형태로 추가할 변수.
        const $replyList = document.getElementById('replyList');

        //게시글 상세보기 화면에 처음 진입했을시 댓글 리스트를 한 번 불러오지.
        getList(1,true);
    

        //댓글 목록을 가져올 함수.
        //getList 매개값으로 뭘 줄거니?
        //요청된 페이지 번호와,화면을 리셋할 것인지의 여부를 bool타입의 reset 으로 받겠습니다.
        //비동기 방식이기 때문에 페이지가 그대로 계속 머물면서 댓글이 밑에 쌓입니다.
        //때에 따라서는 댓글을 계속 누적시키는 것이 아닌,화면을 초기화하고 새롭게 보여줘야 할 때가 있다.
        //reset ->true:페이지를 리셋해서 새롭게 그려내기,reset->false:누적해서 쌀기
        function getList(pageNum,reset){
            ...
            .then(data =>{
                ...
                //더보기 버튼 배치 판단
                //현재 페이지번호 * 이번 요청으로 받은 댓글수 > 전체댓글수 -> 더보기 없어도 됨.
                ...
                //replyList 의 개수만틈 태그를 문자열 형태로 직접 그림.
                //중간중간에 들어가야 할 글쓴이,날짜,댓글 내용은 목록에서 꺼내서 표헌.
                ...
                //id가 replyList 라면 div 영역에 문자열 형식으로 모든 댓글을 추가.
                //insertAdjacentHTML:인접한 곳에 HTML 을 삽입하는 함수
                //(position,문자열 형태의 HTML)
                if(!reset){
        ...
        이 때는 이미 실제로 존재하는 #replylist 에 이벤트를 등록하고, 이벤트를 자식에게 위임하여
        사용하는 addEventListener 를 통해 처리해야 합니다.
        */
        document.getElementById('replyList').addEventListener('click',e => {
            e.preventDefault();//태그의 고유 기능을 중지
            
            //1.이벤트가 발생한 target 이 a태그가 아니라면 이벤트 강제 종료.
            if(!e.target.matches('a')) return;
            
            //2.A태그가 두개(수정,삭제)이므로 어떤 링크인지 확인해야 한다.
            //댓글이 여러 개-> 수정,삭제가 발생하는 댓글이 몇 번인지도 확인해야 한다.
            const rno = e.target.getAttribute('href');
            console.log('댓글 번호는 : ' , rno);
            ...
            console.log('댓글 내용은 : ' , content);

            //3.모달창 하나를 이용해서 상황에 맞게 수정/삭제 모달을 제공해야 한다.
            //조건문을 작성(수정 혹은 삭제에 따라 모달 디자인을 조정)
            if(e.target.classList.contains('replyModify')){
                ...
                //어쩔수 없이 제이쿼리를 이용하여 bootstrap모달을 여는 방법.
