	...
	<script>
		...
		function regist(){
			...
			if(file !== 'jpg' && file !== 'png' && file !== 'jpeg' && file !== 'bmp'){
				alert('이미지 화일(jpc,png,jpeg,bmp) 만 등록이 가능합니다.');
				return;
			}


			/*
			비동기 방식 요청에서 Form 을 생성해서 보내주는 방법
			->FormData 객체를 활용합니다.
			무조건 이 방식이 옳은 것은 아닙니다.FormData 는 비동기 방식이 꼭 필요한 경우에만 사용합니다.
			...
			*/
			...
			console.log('files : ' , $data.files);//파일 태그에 담긴 파일 정보를 확인하는 프로퍼티
			console.log('files index 지정 : ' , $data.files[0]);//사용자가 첨부한 최종 파일의 정보
			...
			fetch('${pageContext.request.contextPath}/snsboard/' + page)
				...
				.then(list => {
					...
					for( board of list){
                        ...
                        <div class="link-inner">`;
							console.log('좋아요가 이상해 : board.bno : likeList ' ,board.bno , likeList );
							if(likeList){
