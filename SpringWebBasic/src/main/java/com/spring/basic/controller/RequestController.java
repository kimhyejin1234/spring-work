...
//자동으로 스프링 컨테이너에 해당 클래스의 빈을 등록하는 아노테니션
//빈을 등록해 나야(객체가 생성되어 있어야) HandlerMapping 이 이 클래스의 객체를 검색할 수 있을 것이다.
@Controller
@RequestMapping("/request") //컨트롤러 자체에 공통된 uri 맴핑
...
//	@RequestMapping(value="/request/basic01",method = RequestMethod.GET)
	...
	public String basicGet() {
		System.out.println("/basic01 요청이들어옴! : GET 방식!!");
		return "request/req-ex01";
	}
//	@RequestMapping(value="/request/basic01", method = RequestMethod.POST)
	@PostMapping("/basic01")
	public String basicPost() {
		System.out.println("/basic01 요청이들어옴! : POST 방식!! ");
		return "request/req-ex01";
	}
	...
	//같은 주소를 사용하는 것이 가능합니다ㅡ.(GET -> 화면 처리 , POST -> 입력값 처리)
	
	/*
	 #스프링에서 요청과 함께 전달될 데이터를 처리하는 방식
	 
	 1. 전통정직 jsp/servler 방식의 파라미터 읽기 처리 방법.
	 - HttpServletRequest 객체를 활용(우리가 jsp 에서 사용하던 방식)
	 -> 스프링에서는 잘 사용하지 않음.
	...
	*/
	 ...
	 *2. @RequestParam 아노테이션을 이용한 요청 파라미터 처리.
	 ...
	 파라미터 변수명과 값을 받을 변수명을 동일하게 작성하면 @RequestParam 생력 가능. 
	@PostMapping("/join")
	public void register(
			String userId,
			String userPw,
	...
	*/
	...
	/*
	 ...
	 - VO 클래스의 필드는 파라미터 변수명과 동일하세 작성합니다.(setter 메서드를 호출)
	 
	 #커멘드 객체:사용자의 입력을 담기위해 설계되고, 특정 검증 로직이나 비즈니스 로직을
