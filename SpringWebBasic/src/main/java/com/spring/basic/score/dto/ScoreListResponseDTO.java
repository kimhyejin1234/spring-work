...
//Entity 는 요청이나 응답을 담는 값으로 쓰지 말라 했죠?
//데이터베이스에서 조회된 값을 화면으로 응답할 때, 해당 화면이 무엇이냐에 따라
//조회된 값을 가공하거나 추가하너가 제외하고 전달해야 할 필요성이 있기 때문에
//그에 맞는 응답 DTO 를 생성해서 전달하는 것이 각자의 역할을 침해하지 않는 설계일 것입니다.
...
public class ScoreListResponseDTO {
	...
	private Grade grade;
	 ...
	 private String makeMaskingName(String originaName) {
		 //valueOf():다른 타입을 문자열로 변환하는 메서드
		 String maskingName = String.valueOf(originaName.charAt(0));
		 for(int i=1;i<originaName.length();i++) {
			 maskingName +=  "*";
		 }
		 return maskingName;
	 }
