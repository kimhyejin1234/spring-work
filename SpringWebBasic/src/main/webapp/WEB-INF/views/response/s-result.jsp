...
<body>
	...
	<script >
	/*
		브라우저가 HTML 을 해석하고, css 를 해석하고, javascript 코드를 실행해서
		화면에 표현하는 과정을 렌더링 이라 합니다.
		지금 우리가 작성하는 파일 형태->jsp->서버 내에서 클래스로 변환->응답은 HTML
		jsp 파일이 클래스로 변환하는 과정에서 작성한 EL 문법은 자바코드로 변환-> 값을 표현
		->응답은 HTML 로 표현.
		EL표현식이 서버에서 먼저 평가되고 그 결과를 클라이언트에게 전달하여 브라우저로 표현하기 때문에
		script 에서도 EL 표현이 가능합니다.
		javaScript 에서 EL 표현식을 작성할 때는 문자열  로 감싸주세요.
	*/
		const result = '${result}';
		...
		switch (result) {
		case 'f-id':
			$h1.textContent = "아이디가 존재하지 않아요!!";
			break;
		case 'f-pw':
			$h1.textContent = "비밀번호가 틀렸어요!!";
			break;
		case 'success':
